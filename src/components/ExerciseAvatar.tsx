...
const ExerciseAvatar = ({ exerciseId, currentRep, isPaused, mode, sensorData, isSensorConnected, trackedLeg }: ExerciseAvatarProps) => {
  ...
  const bodyGroupRef = useRef<Group>(null); // Wraps pelvis + legs; receives pelvis sensor rotation
  ...
  useFrame((state) => {
    ...
    if (mode === 'live' && sensorData && isSensorConnected) {
      ...
      // POSE COMPENSATION: Correct sensor data for the exercise's starting pose.
      //
      // The bodyGroupRef now wraps pelvis + legs, so the pelvis sensor rotation propagates
      // to all body parts (matching GaitAvatar's hierarchy).
      //
      // TWO scenarios determine whether a -PI/2 X correction is needed:
      //
      // SCENARIO A - NO CALIBRATION (raw sensor data):
      //   IMU sensors output in WORLD/GRAVITY space where "lying flat" ≈ identity.
      //   For LYING exercises: parent group rotation [-PI/2, 0, 0] naturally aligns → no correction.
      //   For STANDING/SITTING exercises: parent group is identity → apply -PI/2 correction.
      //
      // SCENARIO B - AFTER GAIT CALIBRATION (calibrated sensor data):
      //   Calibration normalizes "standing" ≈ identity → already correct for standing avatar.
      //   No correction needed for any pose.
      const exerciseDef_ = exerciseDefinitions[exerciseId] || exerciseDefinitions["1"];
      const currentPose = exerciseDef_.startingPose;

      // Apply -PI/2 correction ONLY for standing/sitting WITHOUT calibration.
      // After calibration, sensor data is already in "standing = identity" frame.
      ...
      const correctSensorQ = (worldQ: Quaternion): Quaternion => {
        return sensorCorrectionQ.clone().multiply(worldQ);
      };

      // 1. Apply Pelvis Orientation to BODY GROUP (so it propagates to legs)
      //    The bodyGroupRef wraps pelvis mesh + both legs. By rotating it,
      //    the entire body (including legs) follows the pelvis sensor orientation.
      //    This matches GaitAvatar's hierarchy where pelvis IS the parent of legs.
      const pelvisWorldQ = sensorDataMapper.toThreeQuaternion(processed.sensors.pelvis);
      const pelvisQ = correctSensorQ(pelvisWorldQ);
      if (bodyGroupRef.current) {
        bodyGroupRef.current.quaternion.copy(pelvisQ);
      }
      // Reset pelvis mesh to identity (body orientation is now handled by bodyGroupRef)
      if (pelvisMeshRef.current) {
        pelvisMeshRef.current.quaternion.set(0, 0, 0, 1);
      }

      // 2. Apply RELATIVE Thigh Orientations (relative to pelvis)
      if (rightUpperLegRef.current) {
        const thighWorldQ = sensorDataMapper.toThreeQuaternion(processed.sensors.right_thigh);
        const thighLocalQ = correctSensorQ(thighWorldQ);
        const relativeThighQ = pelvisQ.clone().invert().multiply(thighLocalQ);
        rightUpperLegRef.current.quaternion.copy(relativeThighQ);
      }
      if (leftUpperLegRef.current) {
        const thighWorldQ = sensorDataMapper.toThreeQuaternion(processed.sensors.left_thigh);
        const thighLocalQ = correctSensorQ(thighWorldQ);
        const relativeThighQ = pelvisQ.clone().invert().multiply(thighLocalQ);
      ...
      if (rightKneeRef.current) {
        const thighWorldQ = sensorDataMapper.toThreeQuaternion(processed.sensors.right_thigh);
        const thighLocalQ = correctSensorQ(thighWorldQ);
        const shinWorldQ = sensorDataMapper.toThreeQuaternion(processed.sensors.right_shin);
        const shinLocalQ = correctSensorQ(shinWorldQ);
        const relativeShinQ = thighLocalQ.clone().invert().multiply(shinLocalQ);
        rightKneeRef.current.quaternion.copy(relativeShinQ);
      }
      if (leftKneeRef.current) {
        const thighWorldQ = sensorDataMapper.toThreeQuaternion(processed.sensors.left_thigh);
        const thighLocalQ = correctSensorQ(thighWorldQ);
        const shinWorldQ = sensorDataMapper.toThreeQuaternion(processed.sensors.left_shin);
        const shinLocalQ = correctSensorQ(shinWorldQ);
        const relativeShinQ = thighLocalQ.clone().invert().multiply(shinLocalQ);
    ...
    // Reset bodyGroupRef in demo mode (body pose is controlled by group's static rotation)
    if (bodyGroupRef.current) {
      bodyGroupRef.current.quaternion.set(0, 0, 0, 1); // identity
    }
  ...
  return (
    <>
      ...
      <group ref={groupRef} position={pelvisPosition} rotation={pelvisRotation}>
        {/* Body Group: wraps pelvis + legs so pelvis sensor rotation propagates to legs */}
